...
test("ambiguous purpose uses an automatic blocking bottom sheet with one-step continuation",()=>{const source=readFileSync("src/components/PawarnaGenerator.tsx","utf8");for(const rule of ['setSheet("purpose")','sheet==="purpose"','Pawarna perlukan satu maklumat','Produk ni sebenarnya untuk apa?','autoFocus','Simpan & teruskan','Betulkan produk','setStep(1)'])assert.ok(source.includes(rule),rule);assert.ok(!source.includes("Produk ni kegunaan utama dia apa?"));});
