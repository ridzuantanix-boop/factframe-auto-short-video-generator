...
Phase 5.1 classifies extracted entities before applying preservation rules. Only a real named `PERSON` is subject to strict identity retention; place and organisation compression becomes a soft warning unless it changes the event, while essential ages, tolls, missing counts, legal outcomes, negation and unresolved/reported status remain hard failures. Dates may be shortened or omitted but never changed or invented. Legal proceedings are distinguished from the underlying event—for example, seven murder trials cannot become seven murders. Corrective retries name the failed claim and exact constraint rather than repeating the generic prompt.
