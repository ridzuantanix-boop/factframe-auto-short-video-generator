...
`DISCOVERED` rows keep research/visual/narrative scores null. Archive ingestion promotes to `PARTIAL` only after one usable persisted source and one extracted event/claim. Enrichment alone may promote to `READY` for its declared evidence-supported duration: normally at least two distinct useful claims (or one rare internally complete claim), 100% claim linkage, zero unsupported claims, grounded opening/ending, completeness ≥0.85, at least about eight seconds/20 words of unique narration, coherent source dates/entities, clean Malay, and verification status NOT_REQUIRED or VERIFIED. Unresolved source-time states enter bounded historical follow-up; mutable current facts carry TTL/staleness fields. A single strong archival source can support a reported micro-story only when no unresolved follow-up is outstanding; longer requests are capped rather than padded. Visual readiness is deliberately separate. `HIDDEN` is sticky and reserved for invalid/noisy/manual suppression.
