...
const first=await fetch(origin+"/api/factory");if(!first.ok)throw Error(`factory ${first.status}`);const cookie=first.headers.get("set-cookie")?.split(";")[0];if(!cookie)throw Error("session cookie missing");
const headers={origin,cookie,"content-type":"application/json"};
const created=await fetch(origin+"/api/products",{method:"POST",headers:{...headers,"idempotency-key":crypto.randomUUID()},body:JSON.stringify({images:[image],product_title:title,mode:"Auto",instructions:""})});if(created.status!==202)throw Error(`product ${created.status}: ${await created.text()}`);const id=(await created.json()).product.id;
let product;
