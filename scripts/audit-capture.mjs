...
const output=path.join(root,"docs/audit"),dist=path.join(root,"dist-cloud");
...
const research={status:"unverified",sources:[],evidence:[],queries:[],search_html:"",note:"Fixture audit tempatan. Tiada research web atau generation berbayar dijalankan."};
...
const state=()=>({paused:false,ready:{gemini:true,nexabot:true,worker:true},products:productExists?[project]:[],jobs:jobExists?[{id:jobId,stage:completed?"completed":"processing",created_at:timestamp,updated_at:timestamp,product:analysis,research,plan,settings,retry_count:0,duration_seconds:10,segment_number:1,has_avatar:false,image_count:1,thumbnail_url:"/audit-fixture/product.png",...(completed?{video_url:"/audit-fixture/result.webm"}:{})}]:[],usage:jobExists?[{id:jobId,name:analysis.name,attempts:1,recorded_cost:.5,stage:completed?"completed":"processing"}]:[]});
try{
  ...
  await page.goto(origin+"/#create");await page.getByLabel("Upload gambar produk",{exact:true}).setInputFiles(path.join(output,"fixture-product.png"));
  ...
  await page.getByRole("button",{name:"Teruskan",exact:true}).click();await page.getByRole("heading",{name:"Style",exact:true}).waitFor();await capture(page,"04-video-style-fixture","fixture");
  ...
  await page.getByRole("button",{name:"Curiosity",exact:true}).click();await page.getByRole("button",{name:"Teruskan",exact:true}).click();await page.getByRole("heading",{name:"Creator & Voice",exact:true}).waitFor();
  ...
  const aurat=await page.getByRole("group",{name:"Liputan aurat",exact:true}).evaluate(el=>el.getBoundingClientRect().top+window.scrollY-170);await capture(page,"06c-aurat-fixture","fixture",{full:false,scroll:aurat});
  await page.getByRole("button",{name:"Teruskan",exact:true}).click();await page.getByRole("heading",{name:"Review",exact:true}).waitFor();await capture(page,"07-review-fixture","fixture");
  ...
  await page.getByRole("navigation",{name:"Navigasi utama"}).getByRole("button",{name:"Projects",exact:true}).click();await page.getByRole("heading",{name:"Projects",exact:true}).waitFor();await capture(page,"10-projects-fixture","fixture");
  ...
  await page.getByRole("navigation",{name:"Navigasi utama"}).getByRole("button",{name:"Credits",exact:true}).click();await capture(page,"12-credits-fixture","fixture");
  await page.getByRole("navigation",{name:"Navigasi utama"}).getByRole("button",{name:"Profile",exact:true}).click();await capture(page,"13-profile-fixture","fixture");
  ...
  await writeFile(path.join(output,"capture-index.json"),JSON.stringify({captured_at:new Date().toISOString(),live,viewport,production_version:"f6a4747b-e16c-4460-b3c1-cf8b37e41a98",production_paused:true,production_assets_verified:assetChecks,paid_api_requests:0,fixture_notice:"Fixture images, analysis, scripts, jobs and video are synthetic UI audit data. No provider execution. No app source/copy/CSS changes.",captures,failures,blocked_requests:blocked,runtime_sha256:runtime},null,2)+"\n");
