...
let lastPrompt = ""; let submissions = 0; let analyses = 0; let searches = 0; let planCalls=0; let mode = "success"; let dirtyReference=false;let weakPlans=false;let unsafePlans=false;let wrongGrounding=false;
...
const mock = createServer(async (req, res) => {
  ...
  if (req.url.includes(":generateContent")) {
    ...
    } else if(content.includes("SURFACE HUMANIZER V1.5"))value=content.includes("Dr Lan")?{hook:"Rambut makin gugur bila sikat?",script:"Rambut makin gugur bila sikat? Jangan buat tak tahu. Cuba tengok Dr Lan ni. Klik link kat bawah sekarang.",cta:"Klik link kat bawah sekarang."}:{hook:"Apa yang buat buku ni mudah dicam?",script:"Apa yang buat buku ni mudah dicam? Warna biru dengan tajuk dekat depan. Klik link kat bawah sekarang.",cta:"Klik link kat bawah sekarang."};
    else if (content.includes("Audit this Malay script")) value = { safety_safe: !wrongGrounding, quality_approved: !weakPlans, reason: wrongGrounding?"Problem and solution do not match product":weakPlans?"Safe but needs stronger tension":"Supported" };
    else if(content.includes("VOICE IS OFF:"))value={...plan,script:"",cta:"",hook:"Show closed blue cover"};else if(unsafePlans)value={...plan,hook:"Aku dah guna Dr Lan.",script:"Aku dah guna Dr Lan. Memang berkesan untuk rambut. Klik link kat bawah sekarang.",cta:"Klik link kat bawah sekarang."};else if(wrongGrounding)value={...plan,hook:"Kulit kepala gatal dan berminyak?",script:"Kulit kepala gatal dan berminyak? Dr Lan untuk penjagaan kelemumur. Klik link kat bawah sekarang.",cta:"Klik link kat bawah sekarang."};else if(weakPlans)value={...plan,hook:"Rambut makin gugur?",script:"Rambut makin gugur? Dr Lan untuk penjagaan rambut menipis. Klik link kat bawah sekarang.",cta:"Klik link kat bawah sekarang."};else{const variants=[
...
try {
  ...
  Object.assign(product,{name:"Dr Lan",brand:"Dr Lan",category:"hair care",visible_text:"FOR THINNING HAIR AND HAIR LOSS",description:"Hair water spray",observed_features:["Packaging states FOR THINNING HAIR AND HAIR LOSS"],primary_function:"penjagaan rambut menipis dan rambut gugur",confidence:"high",uncertainty:"",search_query:"Dr Lan hair water spray"});
  const drResponse=await productRequest({images:[image]},crypto.randomUUID());assert.equal(drResponse.status,202);const drId=(await drResponse.json()).product.id;
  for(let i=0;i<120;i++){const p=(await list(cookie)).products.find(p=>p.id===drId);if(p.stage==="ready")break;if(p.stage==="failed")throw Error(p.error);await delay(1000);}
  const drSettings={...settings,productId:drId,videoStyle:"problem_solution",angle:"auto"};weakPlans=true;const imperfect=await scriptFor(drId,drSettings);assert.ok(imperfect.approved_script,"Safe imperfect Dr Lan draft must remain available");
  weakPlans=false;unsafePlans=true;const safeFallback=await scriptFor(drId,drSettings);unsafePlans=false;assert.ok(safeFallback.approved_script);assert.doesNotMatch(safeFallback.approved_script,/aku dah guna|memang berkesan/i,"Unsafe drafts must never escape fallback");assert.equal(submissions,5);
  wrongGrounding=true;const groundedFallback=await scriptFor(drId,drSettings);wrongGrounding=false;assert.ok(groundedFallback.approved_script);assert.doesNotMatch(groundedFallback.approved_script,/kulit kepala|kelemumur/i,"Unrelated problem/solution must never escape factual audit");
  console.log("PASS: safe imperfect /script returns 200; all-unsafe drafts use a non-empty semantic safe fallback.");
