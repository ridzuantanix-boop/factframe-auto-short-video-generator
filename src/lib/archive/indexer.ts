...
function clusterCandidate(cluster: ArchiveCluster, queryByUrl: Map<string, string>): StoryCandidateInput {
  ...
  return {
    ...
    metadata: { categories: ["archive"], archiveDerived: true, archiveClusterKey: cluster.key, originProviders: providers,
      historicalContext: representative.historicalContext, geographyConfidence: specificLocation === "Malaysia / Malaya" ? "LOW" : "MEDIUM",
      geographyEvidence: locations.map((location) => ({ type: "ARCHIVE_EXTRACTED_LOCATION", value: location })),
      classificationVersion: "3-archive-evidence", mysteryPotential: ["PARANORMAL_REPORT", "UNEXPLAINED_EVENT", "CRIME_MYSTERY", "MYSTERIOUS_DEATH", "DISAPPEARANCE"].includes(representative.incidentType) ? "HIGH" : "MEDIUM",
