...
export function speechInstructions(style?: GenerationSettings["voiceStyle"]) {
  ...
  return `Target ${policy.target} spoken words TOTAL including CTA; maximum ${policy.max}. Default 18–23; natural/soft sell strongly prefer 18–22. Shorter natural scripts are welcome: never pad to reach a minimum. Prioritise natural human pacing, understandable hook, ONE key point, then a complete CTA. CTA must finish naturally before the video ends. End exactly "${SPOKEN_CTA}"; CTA field must equal it. Complete script exactly once, no extra sentences.`;
}
export function validSpeech(plan: ContentPlan, settings?: GenerationSettings) {
  ...
  return words > 4 && words <= speechPolicy(settings?.voiceStyle).max && script.endsWith(SPOKEN_CTA)
    && plan.cta === SPOKEN_CTA && script.startsWith(plan.hook) && script.split(SPOKEN_CTA).length === 2
