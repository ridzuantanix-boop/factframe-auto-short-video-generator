...
export const STRONG_SPOKEN_CTA = "Klik link kat bawah sekarang.";
export const SPOKEN_CTAS = [STRONG_SPOKEN_CTA, SPOKEN_CTA,"Tekan link kat bawah.","Kalau nak tengok, klik link kat bawah.","Pergi tengok dekat link bawah."] as const;
...
export const LANGUAGE_AUDIO_LOCK = `Speak only Bahasa Melayu Malaysia: Malaysian Malay only, conversational local language; do not drift into Indonesian or unnecessarily formal Malay. Full readable words, no invented claims or extra improvised sentences. Audible human Malay speech, natural delivery, clear foreground voice, no robotic TTS feeling or loud music competing with speech. Complete the supplied script exactly once and never add, remove, rewrite, paraphrase or expand any sentence. Realistic lip sync when a creator is visible. Preserve the exact natural Malaysian CTA supplied in the approved script; preferred AI default is "${STRONG_SPOKEN_CTA}" and "${SPOKEN_CTA}" remains valid. Never change it to "Klik pautan." If the user-approved script omits the CTA, do not add one. Do not rush or truncate a supplied CTA; end naturally only after the approved script is complete.`;
