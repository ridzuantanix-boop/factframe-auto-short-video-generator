...
export type AiGateway = {
  model: string;
  rewrite(claims: ResearchClaim[], sources: StoredStorySource[], candidate: StoryCandidate, correction?: string[]): Promise<{ outputs: AiClaimRewrite[]; usage: AiUsage }>;
...
export type EnrichAiResult = { package: Awaited<ReturnType<typeof persistResearchClaims>>; usage: AiUsage; claimsSent: number; rewritten: number; rejected: number; retries: number; cacheHits: number; storyCacheHit: boolean; storyGenerated: boolean; sourcePagesAttempted: number; sourcesDeepened: number; failures: Array<{ claimId?: string; attemptedSpokenText?: string; reasons: string[] }> };

export async function enrichCandidateWithAi(candidateId: string, gateway: AiGateway | null = createGeminiGateway(), store: StoryStore = createStoryStore()): Promise<EnrichAiResult> {
  ...
  const callRewrite = async (items: ResearchClaim[], correction?: string[]) => { usageTotal.requests += 1; const response = await gateway!.rewrite(items, sources, candidate, correction);
    usageTotal.inputTokens += response.usage.inputTokens; usageTotal.outputTokens += response.usage.outputTokens; return response; };
  const callStory = async (items: ResearchClaim[]) => { usageTotal.requests += 1; const response = await gateway!.writeStory(items, candidate);
    usageTotal.inputTokens += response.usage.inputTokens; usageTotal.outputTokens += response.usage.outputTokens; return response; };
  const claims = existing.claims.map((claim) => ({ ...claim, rewriteMethod: claim.rewriteMethod ?? (claim.spokenText ? "DETERMINISTIC" : "NONE"),
    ...
    validationResult: claim.validationResult ?? null })); const cached = claims.filter((claim) => claim.rewriteMethod === "GEMINI" && claim.spokenText && claim.validationResult?.valid && claim.validationVersion === CLAIM_VALIDATION_VERSION);
  const pending = claims.filter((claim) => !claim.spokenText && claim.confidence !== "LOW" && claim.ocrQuality >= .65); const failures: Array<{ claimId?: string; attemptedSpokenText?: string; reasons: string[] }> = [];
  ...
  if (gateway && pending.length) {
    try { let response = await callRewrite(pending); let validation = validateRewriteSet(pending, response.outputs);
      ...
      if (validation.rejected.length) {
        retries += 1; const retryClaims = validation.rejected.flatMap((item) => item.claim ? [item.claim] : []);
        if (retryClaims.length) { response = await callRewrite(retryClaims, validation.rejected.flatMap((item) => item.result.reasons));
          validation = validateRewriteSet(retryClaims, response.outputs); accept(validation.accepted); }
        rejected += validation.rejected.length; failures.push(...validation.rejected.map((item) => ({ claimId: item.claim?.id, attemptedSpokenText: item.output?.spokenText, reasons: item.result.reasons })));
      }
  ...
  else if (gateway && useful.length >= 3 && quality.passes && coherent.confidence !== "LOW" && !existing.requiresCurrentVerification) {
  ...
  return { package: packageValue, usage: usageTotal, claimsSent: pending.length, rewritten, rejected, retries, cacheHits: cached.length, storyCacheHit,
