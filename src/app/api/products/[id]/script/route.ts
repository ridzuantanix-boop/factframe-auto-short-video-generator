...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 try{sameOrigin(request);const {id}=await params,saved=getProduct(id);if(!saved||saved.project.owner!==await ownerId())return new Response(null,{status:404});
  ...
  p.script_history=history;p.route_history=routeHistory;p.script_draft={plan:{...plan,script_source:"ai",script_settings_hash:settings_hash,script_generated_at:generated_at},settings_hash,generated_at};saveProduct(p);
