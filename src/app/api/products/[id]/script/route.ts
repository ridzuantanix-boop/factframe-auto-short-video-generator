...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
 try{sameOrigin(request);const {id}=await params,saved=getProduct(id);if(!saved||saved.project.owner!==await ownerId())return new Response(null,{status:404});
  ...
  const history=[...(p.script_history||[]),...(p.script_draft?.plan.script?[p.script_draft.plan.script]:[])].slice(-5),input=projectInput({...saved.input,settings,instructions,angle_seed:crypto.randomUUID(),previous_hook:p.script_draft?.plan.hook,previous_scripts:history},p),reused=body.source_job?getJob(String(body.source_job)):undefined;if(body.source_job&&(!reused||reused.owner!==p.owner||!reused.plan))return new Response(null,{status:404});
  ...
  p.script_history=history;p.script_draft={plan:{...plan,script_source:"ai",script_settings_hash:settings_hash,script_generated_at:generated_at},settings_hash,generated_at};saveProduct(p);
