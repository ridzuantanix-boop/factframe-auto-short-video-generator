...
const emotionalRoutes=["pain escalation","relatable daily frustration","consequence and tension","curiosity","FOMO and social interest","direct recommendation"];
function emotionalRoute(input:JobInput){
  if(!input.previous_scripts?.length&&input.angle_seed==="default")return "strongest grounded conversion route";
  const seed=[input.angle_seed,...(input.previous_scripts||[])].join("|");
  return emotionalRoutes[[...seed].reduce((sum,char)=>sum+char.charCodeAt(0),0)%emotionalRoutes.length];
}
export async function createPlan(input: JobInput, product: ProductAnalysis, research: Research): Promise<ContentPlan> {
  ...
  const route=emotionalRoute(input);
  ...
  const controls = [...globalPromptLocks(voice), ...(input.settings ? controlModules(input.settings, !!input.avatar) : []),resolverRules,`REQUIRED EMOTIONAL ROUTE FOR THIS VARIATION: ${route}. Regeneration must change the emotional sales strategy and sentence structure, not merely swap synonyms.`].join("\n");
  const facts = { product, research: { evidence: research.evidence, note: research.note }, notes: input.instructions };
  let feedback = "",bestSafe:{plan:ContentPlan;score:number}|undefined;
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if(safetyProblems.length){feedback=`HARD SAFETY REWRITE REQUIRED: ${safetyProblems.join(", ")}. Never retain these claims. Replace them with supported packaging positioning or safe category context.`;continue;}
    ...
    catch{review={safety_safe:false,quality_approved:false,reason:"Secondary factual grounding review unavailable"};}
    if(!review.safety_safe){feedback=`HARD SAFETY REWRITE REQUIRED: ${review.reason}`;continue;}
    const score=qualityProblems.length+(review.quality_approved?0:1);if(!bestSafe||score<bestSafe.score)bestSafe={plan,score};
    if(review.quality_approved&&qualityProblems.length===0)return {...plan,video_prompt:""};
