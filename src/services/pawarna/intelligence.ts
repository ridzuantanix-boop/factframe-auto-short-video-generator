...
export async function createPlan(input: JobInput, product: ProductAnalysis, research: Research): Promise<ContentPlan> {
  ...
  const resolverRules="AFFILIATE SALES SCRIPT DIRECTOR V1.1: sound like a real Malaysian TikTok creator speaking naturally to one person to stop scroll, create interest/desire, and earn the click. Never sound like a catalogue, corporation, formal translation, AI assistant, or compliance notice. Internally choose the strongest supported consumer territory, then use PAIN/DESIRE → TENSION/CONSEQUENCE → PRODUCT AS ANSWER → GENERAL FOMO/REASON TO CHECK → CTA. Fit it naturally; do not mechanically force five sentences. Short spoken phrases, direct address and Malaysian words such as korang, yang ni, makin, memang, kalau and dah are welcome when natural, never as a repeated template. Avoid 'Produk ini sesuai/mengandungi/direka/merupakan', 'Berdasarkan maklumat', 'Bagi mereka yang', and 'Sekiranya anda'. Embed the product naturally; do not read its full packaging name or force ingredients. Default AI CTA is 'Klik link kat bawah sekarang.' Auto chooses the strongest conversion territory, not the easiest visual fact. For Problem → Solution, start with specific self-identifying friction and add safe tension before introducing the product. General non-numeric social-interest FOMO such as people looking at or searching this kind of product is allowed; never invent sales, rating, review counts, ranking, stock, price, deadline or scarcity. Review evidence must remain attributed ('Ada yang review kata...'), never objective fact. Never invent creator experience. Use product reasoning even without Search: packaging/listing/title/URL/category knowledge, while preserving source semantics. Explicit packaging positioning such as FOR THINNING HAIR AND HAIR LOSS may be used strongly as intended positioning, never as treatment, guaranteed outcome, hair regrowth or efficacy. Shape/container/colour remain visual fidelity, not default sales copy. Safety constrains facts without destroying energy or persuasion.";
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try{review=await json(`Audit this Malay script and visual direction BEFORE generation. Return safety_safe=false ONLY for unsupported factual/medical efficacy, fabricated creator experience, fabricated exact social proof/price/promo/scarcity, invalid evidence attribution or another hard factual-safety breach. Return quality_approved=false for weak hook, catalogue tone, weak tension/FOMO/sales intent, unnatural Malay, or insufficient regeneration variation; those are quality concerns, not safety. Ask whether a real Malaysian TikTok affiliate would naturally say it aloud and whether sentence one earns sentence two. Score Hook Strength, Relatability, Human Speech, Tension, Product Relevance, FOMO/Desire, Evidence Safety, CTA Strength and 10-second Speakability. General non-numeric curiosity/social-interest FOMO is allowed. Conservative exact packaging positioning such as "for thinning hair and hair loss" may be used strongly as positioning, never converted into treatment, regrowth, result or guarantee. Review evidence remains attributed. Data: ${JSON.stringify({facts,plan,controls,previous_hook:input.previous_hook,recent_scripts:input.previous_scripts||[]})}`,object({safety_safe:{type:"boolean"},quality_approved:{type:"boolean"},reason:string}));}
    catch{review={safety_safe:true,quality_approved:false,reason:"Secondary conversion review unavailable"};}
